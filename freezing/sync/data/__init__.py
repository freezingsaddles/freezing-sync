...
class StravaClientForAthlete(Client):
    ...
    def __init__(
        ...
        if not isinstance(athlete, Athlete):
            ...
            if not athlete:
                raise ValueError(
                    "Athlete ID does not exist in database: {}".format(
                        athlete_id))
        super(
                StravaClientForAthlete,
                self
        ).__init__(access_token=athlete.access_token, rate_limit_requests=True)
    ...
    def refresh_access_token(self, athlete: Athlete):
        ...
        if (athlete.refresh_token is not None):
            ...
            raise ValueError(
                    "athlete %s had no access or refresh token".format(
                        athlete.id))
        if (refresh_token):
            self.logger.info(
                    "saving refresh token for athlete %s",
                    athlete.id
                    )
