...
class ActivitySync(BaseSync):
    ...
    def check_activity(
        ...
        *,
        ...
        activity_end_date = activity.start_date + activity.elapsed_time.timedelta()
    ...
    def write_ride(self, activity: SummaryActivity) -> Ride:
        ...
        if new_ride:
            ...
            if round(ride.distance, 3) != round(
                float(unit_helper.miles(activity.distance.quantity()).magnitude), 3
            ):
                self.logger.info(
                    ...
                    "distance mismatch ({1} != {2})".format(
                        activity,
                        ride.distance,
                        float(unit_helper.miles(activity.distance.quantity()).magnitude),
                    )
                )
