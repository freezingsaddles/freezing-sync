...
from freezing.sync.utils.cache import CachingAthleteObjectFetcher, CachingActivityFetcher, CachingStreamFetcher
...
class TestCachingAthleteObjectFetcher:
    def test_cache_object_json(self, cache_basedir, client):
        ...
        cache_path = fetcher.cache_object_json(athlete_id=athlete_id, object_id=object_id, object_json=object_json)
    ...
    def test_get_cached_object_json(self, cache_basedir, client):
        ...
        object_id = 456
        object_json = {"id": object_id, "name": "Test Activity"}
        fetcher.cache_object_json(athlete_id=athlete_id, object_id=object_id, object_json=object_json)
        cached_data = fetcher.get_cached_object_json(athlete_id=athlete_id, activity_id=object_id)
        assert cached_data == object_json
    ...
    def test_retrieve_object_json(self, mock_download, cache_basedir, client):
        ...
        # Test cache miss
        retrieved_data = fetcher.retrieve_object_json(athlete_id=athlete_id, object_id=object_id, use_cache=True, only_cache=False)
        assert retrieved_data == object_json
        mock_download.assert_called_once_with(athlete_id=athlete_id, object_id=object_id)
...
class TestCachingActivityFetcher:
    ...
    def test_fetch(self, mock_get_activity, cache_basedir, client):
        ...
        fetched_activity = fetcher.fetch(athlete_id=athlete_id, object_id=object_id, use_cache=True, only_cache=False)
...
class TestCachingStreamFetcher:
    ...
    def test_fetch(self, mock_get_activity_streams, cache_basedir, client):
        ...
        streams = [Stream.deserialize(stream, bind_client=client) for stream in streams_json]
        mock_get_activity_streams.return_value = streams

        # Test fetch with cache miss
        fetched_streams = fetcher.fetch(athlete_id=athlete_id, object_id=object_id, use_cache=True, only_cache=False)
